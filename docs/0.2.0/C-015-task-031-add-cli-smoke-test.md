...
- **Objective:** Add an end-to-end smoke test that exercises the CLI against a freshly started TaskAgent API server.
- **Target Files/Scope:** `cli/test/smoke.sh`
- **Token Budget Heuristic:**
  * Estimated Existing Code Context: Medium - this test needs command contracts plus the existing server startup and seed behavior, but can treat server internals as a black box.
  * Dependencies: Completed CLI commands; Go server `go run ./cmd/server`; API key seed command if available.

## Step-by-step Instructions

1. Add `cli/test/smoke.sh` with `#!/usr/bin/env sh` and `set -eu`.
2. Create an isolated temporary directory for `TASKAGENT_HOME` and server database state.
...
5. Run `task login` against the local server.
...
13. Print concise progress to stderr and keep stdout assertions machine-readable.

## Definition of Done

- `sh cli/test/smoke.sh` passes on a clean checkout with Go and CLI dependencies installed.
